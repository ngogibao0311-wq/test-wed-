        ...
        <div class="modal-content form-container" style="max-width: 500px; text-align: left; border-top: 6px solid #3b82f6; border-radius: 12px; padding: 25px;">
            ...
            <div style="max-height: 60vh; overflow-y: auto; font-size: 0.95em; color: #444; line-height: 1.6; padding-right: 10px;">
                ...
                <ul style="margin-top: 5px; padding-left: 20px;">
                    <li><strong>ĐTB Tháng</strong> = Tổng điểm bài hợp lệ / Số bài.</li>
                    <li>Nếu hòa ĐTB, xét tiêu chí phụ: <strong>Số bài điểm 10</strong> > <strong>Số lần vi phạm ít hơn</strong>.</li>
                    <li>Điểm và Xếp hạng được <strong>reset về 0</strong> vào đầu mỗi tháng.</li>
                </ul>
                ...
                <ul style="margin-top: 5px; padding-left: 20px;">
                    <li><strong>Nộp trễ hạn:</strong> Bài làm sẽ không được tính vào ĐTB.</li>
                    <li><strong>Gian lận thi cử:</strong> Thoát toàn màn hình, mở tab khác... Hệ thống tự động thu bài và hủy điểm.</li>
                    <li><strong>Lỗi nộp bài:</strong> Nếu bài yêu cầu đính kèm tệp mà chỉ nhập chữ, hệ thống sẽ chặn nộp.</li>
                </ul>
                ...
                <ul style="margin-top: 5px; padding-left: 20px; list-style-type: none;">
                    <li style="margin-bottom: 8px;">🥇 <strong>Hạng 1:</strong> 1 Rương Kho Báu (Tự chọn mở Coin số lượng lớn hoặc cơ hội rớt Đồ Truyền Thuyết).</li>
                    <li style="margin-bottom: 8px;">🥈 <strong>Hạng 2:</strong> 1 Thẻ giảm giá ngẫu nhiên (Mức giảm từ 5% - 50%).</li>
                    <li style="margin-bottom: 8px;">🥉 <strong>Hạng 3:</strong> Nhận ngay 100 Coin.</li>
                    <li>🎖️ <strong>Hạng 4 trở đi:</strong> Nhận 50 Coin khích lệ (Yêu cầu ĐTB > 0).</li>
                </ul>
            </div>
    ...
    try {
        ...
        students.forEach(student => {
            ...
            studentSubs.forEach(sub => {
                ...
                // Lọc theo tháng (Dựa trên endDate của bài tập)
                if (assign.endDate) {
                    const assignDate = new Date(assign.endDate.replace(" ", "T"));
                    if (assignDate.getMonth() !== currentMonth || assignDate.getFullYear() !== currentYear) return;
                }

                // Tiêu chí 1: Đã chấm điểm, không đang chấm lại
                if (sub.grade === null || sub.grade === undefined || sub.grade === '' || sub.isRegrading) return;

                // Tiêu chí 2 & 3: Lọc vi phạm và nộp trễ (Trừ khi được Giáo viên Tha lỗi - forcePass)
                const isLate = sub.isLateFail || sub.isAutoSubmitted;
                const isCheat = sub.isCheatFail;
                
                if (sub.forcePass) {
                    // Được tha lỗi -> Tính bình thường
                } else if (isLate || isCheat) {
                    violationCount++;
                    return; // Loại khỏi danh sách tính điểm hợp lệ
                }
        ...
        // THUẬT TOÁN SẮP XẾP: 1. ĐTB giảm dần -> 2. Số điểm 10 giảm dần -> 3. Số vi phạm tăng dần
        rankedData.sort((a, b) => {
            if (b.dtb !== a.dtb) return b.dtb - a.dtb;
        ...
        if (rankedData.length === 0) {
            body.innerHTML = '<div style="text-align:center; padding: 30px; color: #64748b; font-style: italic;">Chưa có dữ liệu xếp hạng trong tháng này.</div>';
            return;
        }
        ...
        rankedData.forEach((st, index) => {
            ...
            <div class="rank-row ${rankClass}">
                ...
                <div class="rank-stats">
                    <span class="rank-score">${st.dtb}</span>
                    <span style="font-size: 0.85em;">Điểm 10: <strong>${st.tens}</strong> | Vi phạm: <strong>${st.violations}</strong></span>
                </div>
            </div>`;
        });
